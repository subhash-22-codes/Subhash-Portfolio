       ...
       <motion.div 
        ...
        <div className="space-y-16 md:space-y-24">
          {skillGroups.map((group, groupIdx) => (
            <div key={groupIdx} className="grid grid-cols-1 lg:grid-cols-12 gap-8 items-center border-t border-white/5 pt-16 group/row relative">
              
              <div className="absolute inset-0 bg-white/[0.01] opacity-0 group-hover/row:opacity-100 transition-opacity duration-1000 -mx-10" />

              {/* LEFT: CATEGORY (Drift Left) */}
              <motion.div 
                variants={driftLeft}
                initial="initial"
                whileInView="whileInView"
                viewport={{ once: false, amount: 0.5 }}
                className="lg:col-span-4 flex items-center gap-8 relative z-10"
              >
                <div className="w-16 h-16 rounded-2xl bg-white/[0.03] border border-white/10 flex items-center justify-center text-white/20 group-hover/row:text-white group-hover/row:border-white/30 transition-all duration-700">
                  {group.icon}
                </div>
                <div>
                  <h3 className="font-serif text-3xl md:text-4xl tracking-tight text-white/90">
                    {group.title}
                  </h3>
                  <p className="font-mono text-[9px] uppercase tracking-[0.4em] text-white/20 mt-3">
                    Knowledge // 0{groupIdx + 1}
                  </p>
                </div>
              </motion.div>

              {/* RIGHT: SKILL BADGES (Drift Right) */}
              <motion.div 
                variants={driftRight}
                initial="initial"
                whileInView="whileInView"
                viewport={{ once: false, amount: 0.5 }}
                className="lg:col-span-8 flex flex-wrap gap-3 md:gap-5 relative z-10"
              >
                {group.skills.map((skill, skillIdx) => (
                  <div 
                    key={skillIdx}
                    className="flex items-center gap-4 px-6 py-4 rounded-xl bg-white/[0.02] border border-white/5 hover:border-white/20 hover:bg-white/5 transition-all duration-700 group/badge cursor-default"
                  >
                    <img 
                      src={skill.icon} 
                      alt={skill.name} 
                      className="w-5 h-5 object-contain filter grayscale group-hover/badge:grayscale-0 transition-all duration-500" 
                    />
                    <span className="font-sans text-[10px] font-bold uppercase tracking-[0.2em] text-white/40 group-hover/badge:text-white transition-colors">
                      {skill.name}
                    </span>
                  </div>
                ))}
